    ...
    <div className='min-h-screen flex flex-col bg-slate-900 text-slate-100 font-sans selection:bg-indigo-500 selection:text-white gap-5 pb-4'>
      ...
      <div className="flex flex-col mx-auto px-4 w-full">
        ...
        <h2 className="text-2xl md:text-3xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400">
          Generate Randomized Schulte Tables Instantly
        </h2>
        
        <p className="text-slate-300 text-base md:text-lg leading-relaxed">
          Generate randomized Schulte Tables for attention training, visual search exercises, and cognitive development. Configure the table size and integrate the generated matrix into your applications using a single GraphQL API.
        </p>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 w-full mt-2 text-sm">
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-3 backdrop-blur-sm">
            <span className="block font-semibold text-indigo-400 mb-1">🧠 Cognitive Training</span>
            <span className="text-slate-400 text-xs">Improve attention, concentration, and visual processing.</span>
          </div>
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-3 backdrop-blur-sm">
            <span className="block font-semibold text-indigo-400 mb-1">📐 Configurable Sizes</span>
            <span className="text-slate-400 text-xs">Generate different Schulte Table dimensions.</span>
          </div>
          <div className="bg-slate-800/50 border border-slate-700/50 rounded-xl p-3 backdrop-blur-sm">
            <span className="block font-semibold text-indigo-400 mb-1">⚡ Randomized Layouts</span>
            <span className="text-slate-400 text-xs">Every request returns a newly shuffled table.</span>
          </div>
        </div>
      </div>
      </div>
        ...
        <div className='flex flex-col gap-4 text-left bg-slate-800/40 border border-slate-700/50 rounded-2xl p-5 backdrop-blur-sm shadow-xl'>
          <div className="space-y-1">
            ...
            <p className="text-slate-300 text-sm leading-relaxed">
              You don't have to install any library or perform any complex setup. Simply send a GraphQL request with your preferred table size to generate a randomized Schulte Table.
            </p>
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 max-w-6xl mx-auto px-4 w-full mt-6 ">
        ...
        <div className="flex flex-col gap-4 justify-between bg-slate-800/40 border border-slate-700/50 rounded-2xl p-5 backdrop-blur-sm shadow-xl">
          <div>
          ...
          <div className="bg-slate-950 border border-slate-800 p-4 rounded-xl font-mono text-xs text-indigo-300 space-y-1">
            ...
            <p><span className="text-cyan-400">const</span> size = <span className="text-amber-400">3 | 4 | 5 | 6 | 7 | Custom</span>;</p>
          </div>
    ...
    message
          ...
          <div>
            ...
            <p className="bg-slate-950 border border-slate-800 p-2.5 rounded-lg font-mono text-[11px] break-all text-slate-400">
              <span className="text-pink-400 font-medium">?query=</span>{`{newSchulteTable(size:5){schulteBoard,message}}`}
            </p>
          </div>
        </div>
